...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            - 2번 항목(약점 분석): 반드시 최소 2가지 이상의 약점을 분석하세요. 약점을 지적할 때는 단순히 약점만 나열하지 말고, **반드시 학생부에 기재된 특정 활동 내용(출처 꼬리표 포함)을 먼저 짚어준 뒤**, 이를 보완하기 위한 '구체적인 후속 활동이나 심화 탐구 방안'을 제안하세요.
            
            - 4번 항목(추천 도서 및 연계 활동):
              1) 도서를 나열할 때 반드시 "■ 교과명(또는 영역명):" 이라는 굵은 소제목으로 과목/영역을 확실히 구분하여 먼저 적으세요.
              2) 각 교과명 아래에 도서를 1번부터 차례대로 순차 번호를 매겨 나열하세요.
              3) 도서 1개당 반드시 아래의 2줄 포맷을 무조건 지키세요.
                 [번호]. <실제 책제목> (저자명 저) - 세부 본문 내용을 바탕으로 한 구체적인 도서 소개 및 추천 이유
                 **연계 활동:** (반드시 줄을 바꾸고 이 굵은 제목을 달 것) 학생의 생기부 내용과 매칭하여 구체적으로 어떤 심화 탐구나 활동을 할 수 있는지 제안함.

            위의 모든 규칙과 템플릿을 완벽히 적용하여, 아래 5가지 양식에 맞추어 최종 결과물을 작성해 주세요.
            ### 1. 전공 적합성 및 주요 경쟁력 (테마별 엄선, 평가적 분석 서술, 100% 일치하는 분리 출처, 개조식)
            ### 2. 범용 평가 기준에 비추어 볼 때 보완이 필요한 약점 (특정 활동 선 언급 후 구체적 보완책 제시, 100% 일치하는 분리 출처, 개조식) ※ 부재중인 학년의 기록 부족 지적 불가!
            ### 3. 추천 심화 탐구 주제 및 면접 예상 질문 3가지
            ### 4. 맞춤형 추천 도서 및 연계 활동 제안 (과목명 분류, 도서명(저자) 형식, **연계 활동:** 명시적 서술 필수)
            ### 5. 종합 의견 및 향후 발전 방향
            """
