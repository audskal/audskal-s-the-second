...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            🚨 [분석의 깊이 및 톤앤매너 (매우 중요)] 🚨
            단순히 생기부 내용을 요약하거나 줄거리처럼 나열(Summary)하는 것을 엄격히 금지합니다. 학생의 활동이 전공 적합성, 학업 역량, 발전 가능성 측면에서 **어떤 의미를 가지는지 깊이 있게 '평가'하고 '분석(Analysis)'**해야 합니다.
            - 나쁜 예 (단순 요약): ~활동을 통해 ~를 배우고 ~를 느낌.
            - 좋은 예 (심층 분석): ~활동을 통해 ~를 수행한 점은, 전공에서 요구하는 ~역량을 탁월하게 갖추었음을 입증함.

            🚨 [절대 엄수 - 출력 형식 및 출처 표기 규칙!] 🚨
            1. 🚫 [전체 출처 무단 나열 절대 금지]: 각 섹션의 시작 부분이나 문단 상단에 `■ [1학년 국어, 1학년 수학, 1학년 영어...]` 처럼 생기부에 있는 모든 과목과 활동을 분석 내용 없이 길게 나열하는 것을 절대 금지합니다. 출처는 오직 '테마 소제목 옆'과 '문장 끝'에 엄선된 2~3개만 제한적으로 적어야 합니다.
            2. 이중 출처 표기 및 압축: 한 문단에 모든 과목을 다 때려 넣지 마세요. 테마별로 가장 훌륭한 활동 **2~3개만 엄선**하세요.
            3. 학년/과목을 절대 슬래시(/)나 쉼표(,)로 묶어서 표기하지 마세요. (예: [1, 2학년 진로] ❌ -> [1학년 진로활동, 2학년 진로활동] ⭕)
            4. 특정 대학명 노출 절대 금지: 동국대학교 등 대학 이름이 결과물에 직접 등장하면 안 됩니다.
            5. 개조식 어미 사용: 모든 문장 끝은 '~함', '~임', '~됨', '~판단됨' 으로 명사형 종결할 것.

            💡 [형식 참고용 정답 템플릿 (반드시 이 구조와 톤을 똑같이 따라하세요)]
            ...
            ### 5. 종합 의견 및 향후 발전 방향
            1. 향후 발전 방향:
               1) 관심 분야 확장 및 연결: 현재의 강점인 ~역량을 ~분야로 연결하여 탐구하는 노력이 요망됨.
               2) 실질적 활동 참여: ~등 실질적인 프로젝트 활동에 참여하는 것이 중요함.
            2. 최종 코멘트:
               학생은 높은 학업 역량과 긍정적인 태도를 바탕으로... (종합 평가 마무리)

            위의 모든 규칙과 템플릿을 완벽히 적용하여, 아래 5가지 양식에 맞추어 최종 결과물을 작성해 주세요.
            ### 1. 전공 적합성 및 주요 경쟁력 (테마별 엄선, 평가적 분석 서술, 개별 분리 출처, 개조식)
            ### 2. 범용 평가 기준에 비추어 볼 때 보완이 필요한 약점 (엄선된 약점 분석, 개별 분리 출처, 개조식)
            ### 3. 추천 심화 탐구 주제 및 면접 예상 질문 3가지
            ### 4. 맞춤형 추천 도서 및 연계 활동 제안 (반드시 제공된 도서 목록 활용)
            ### 5. 종합 의견 및 향후 발전 방향 (정답 템플릿의 '향후 발전 방향' 및 '최종 코멘트' 넘버링 구조 반드시 준수)
            """
