...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            # --- 💡 [프롬프트 재구성] 특정 대학명(예: 동국대) 노출 금지 규칙 강력 추가 ---
            prompt = f"""
            ...
            5. 🚫 [매우 중요] 특정 대학명 언급 절대 금지: 제공된 가이드북에 "동국대학교", "서울대학교" 등 특정 대학교 이름이나 단과대학 이름이 적혀있더라도, 분석 결과물에 특정 대학/단과대 이름을 절대 직접적으로 적지 마세요! 대학교 이름을 노출하는 것은 심각한 오류입니다. 
               - 나쁜 예: "동국대학교 바이오시스템대학에서는 물리학을 강조함..." (절대 금지)
               - 좋은 예: "목표 대학(또는 지원 희망 전공)에서는 물리학을 전공 관련 교과로 강조함..." 또는 단순히 "생명공학 분야 진학을 위해서는 물리학적 사고력이 중요하게 평가됨..." 과 같이 순화하여 서술하세요.
...
<div style='text-align: center; color: gray; padding: 20px; font-size: 13px;'>
    🏫 학교생활기록부 분석 시스템 v6.2 (대학명 블라인드 처리 패치)<br>
