...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            # --- 💡 [프롬프트 재구성] 묶음 표기 절대 금지 및 개별 분리 표기 규칙 강력 추가 ---
            prompt = f"""
            ...
            5. 🚫 [매우 중요] 특정 대학명 언급 절대 금지: 제공된 가이드북에 특정 대학교 이름이 있더라도, 결과물에 특정 대학/단과대 이름을 절대 노출하지 마세요. "목표 대학(또는 지원 희망 전공)에서는..." 등으로 순화하세요.

            🚨 [절대 엄수 - 출력 형식 및 출처 표기 규칙! (매우 중요)] 🚨
            1. 이중 출처 표기 (필수): 1번과 2번 항목에서 학생부의 내용을 언급할 때는 **반드시** 해당 내용이 몇 학년 어느 영역(과목/활동)에 있는지 출처를 밝혀야 합니다.
               - 🚫 [금지사항]: 학년이나 과목/활동을 절대 쉼표나 슬래시(/)로 묶어서 합쳐서 표기하지 마세요. (예: [1학년, 2학년 진로활동] -> 금지 ❌ / [1, 2학년 수학/수학I/확률과통계] -> 금지 ❌)
               - ⭕ [필수사항]: 반드시 각 학년과 과목/활동을 개별적으로 온전히 분리하여 나열하세요. (예: [1학년 진로활동, 2학년 진로활동] -> 정상 ⭕ / [1학년 수학, 2학년 수학I, 2학년 확률과 통계] -> 정상 ⭕)
               - 문단 시작 시: `■ **[1학년 진로활동, 2학년 수학I]** 핵심 요약` 형태로 분리하여 표기.
               - 문장 끝 시: 해당 활동의 개별 출처를 문장 끝에 `[1학년 진로활동]` 형태로 꼬리표 달기.
            2. 개조식 어미 사용: 1, 2, 4, 5번 항목의 모든 문장 끝은 '~함', '~임', '~됨', '~판단됨' 등으로 명사형 종결할 것. ('~습니다', '~해요' 절대 금지)

            위의 규칙을 완벽히 지켜서, 아래 5가지 양식 및 순서에 맞추어 최종 결과물을 작성해 주세요.
            ### 1. 목표 대학 및 전공 적합성 주요 경쟁력 (반드시 '개별 분리된 이중 출처 표기' 및 '개조식' 적용)
            ### 2. 목표 대학(또는 범용) 평가 기준에 비추어 볼 때 보완이 필요한 약점 (반드시 '개별 분리된 이중 출처 표기' 및 '개조식' 적용)
...
<div style='text-align: center; color: gray; padding: 20px; font-size: 13px;'>
    🏫 학교생활기록부 분석 시스템 v6.3 (출처 개별 분리 표기 패치)<br>
