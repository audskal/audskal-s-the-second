    ...
    if not api_key:
        ...
        try:
            ...
            if actual_book_data.strip():
                book_instruction = "반드시 아래 제공된 [추천 도서 참고 자료]의 텍스트 안에 '실제로 존재하는 책 제목과 저자'만 추출해서 추천하세요. 자료 안에 적합 책이 없다면 억지로 지어내지 마세요."
            else:
            ...
            # --- 💡 [프롬프트 수정] 불필요한 문구 금지 규칙 1줄 추가 (나머지는 v6.8과 100% 동일) ---
            prompt = f"""
            ...
            과거 분석에서 문단 맨 앞의 [요약 출처 모음]과 본문 문장 끝의 [개별 꼬리표]가 서로 다르거나, 뜬금없는 과목이 무단으로 추가되는 치명적인 오류가 발생했습니다. 이를 막기 위해 아래의 '출처 사전 확정' 프로세스를 무조건 따르세요.
            ...
            2. 100% 일치 강제 (추가/누락 절대 금지):
               ...
               - 🚫 글을 쓰다가 즉흥적으로 제목에 없는 출처(예: [2학년 심리학], [2학년 독서])를 본문에 무단 추가하는 것을 엄격히 금지합니다!
               - 🚫 반대로 제목에 선언해 놓고 본문 꼬리표에서는 빼먹는 것도 엄격히 금지합니다! 집합 A(소제목 출처 목록)와 집합 B(본문 꼬리표들의 합)는 완벽하게 100% 동일해야 합니다.

            3. 특정 대학명 노출 절대 금지: 동국대학교 등 대학 이름이 직접 등장하면 안 됩니다.
            4. 🚫 특정 문구 출력 금지: "'쌤과 함께! 교과 연계 적합書]"와 같은 사이트 카테고리명이나 불필요한 기호는 결과물에 절대 출력하지 마세요.
            5. 개조식 어미 사용: 문장 끝은 '~함', '~임', '~됨', '~판단됨' 으로 명사형 종결할 것.
            ...
            *(주의: 소제목 옆의 [1학년 기술·가정, 2학년 동아리활동]과 본문의 꼬리표 2개가 1:1로 완벽히 일치합니다. 중간에 다른 과목이 절대 끼어들지 않았습니다.)*

            위의 모든 규칙과 템플릿을 완벽히 적용하여, 아래 5가지 양식에 맞추어 최종 결과물을 작성해 주세요.
            ### 1. 전공 적합성 및 주요 경쟁력 (테마별 엄선, 평가적 분석 서술, 100% 일치하는 분리 출처, 개조식)
            ### 2. 범용 평가 기준에 비추어 볼 때 보완이 필요한 약점 (엄선된 약점 분석, 100% 일치하는 분리 출처, 개조식)
