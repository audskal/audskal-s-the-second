...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            # --- 💡 [프롬프트 재구성] 백화점식 나열 완벽 차단 및 핵심 압축 서술 강제화 ---
            prompt = f"""
            ...
            🚨 [절대 엄수 - 분석 및 출력 형식 규칙! (매우 중요)] 🚨
            1번과 2번 항목 작성 시 다음 규칙을 무조건 지키세요.

            1. 🚫 [핵심 압축 서술 - 백화점식 나열 절대 금지!]: 학생부의 모든 과목을 문단에 욱여넣지 마세요. 하나의 테마(문단)당 가장 결정적이고 훌륭한 활동 단 **2~3개**만 엄선하여, 3~4문장 내외로 깊이 있게 압축 분석하세요. 수많은 과목을 나열하는 것은 최악의 분석입니다.
            
            2. 이중 출처 표기 (필수): 
               - 🚫 [묶음 표기 금지]: 학년/과목을 슬래시나 쉼표로 묶지 마세요. (예: [1, 2학년 진로] ❌ -> [1학년 진로, 2학년 진로] ⭕)
               - 🚫 [출처 불일치 금지]: 문단 시작 시 적은 출처 목록과 본문 문장 끝의 개별 꼬리표는 100% 일치해야 합니다.
               - ⭕ [필수 형식]: 문단 시작 시 `■ **[출처 모음]**`은 엄선된 핵심 활동 **최대 2~3개**만 적고, 문장 끝에 각각의 꼬리표를 명확히 달아주세요.

            💡 [출처 표기 및 압축 서술의 올바른 예시]
            ...
            *(이유: 불필요한 과목은 다 쳐내고, 핵심 3개만 골라내어 문단 앞과 문장 끝 출처를 완벽하게 일치시킴)*

            3. 개조식 어미 사용: 1, 2, 4, 5번 항목의 모든 문장 끝은 '~함', '~임', '~됨', '~판단됨' 등으로 명사형 종결할 것. ('~습니다', '~해요' 절대 금지)

            위의 모든 규칙을 완벽히 지켜서, 아래 5가지 양식 및 순서에 맞추어 최종 결과물을 작성해 주세요.
            ### 1. 목표 대학 및 전공 적합성 주요 경쟁력 (테마별 엄선된 2~3개 활동 압축, 출처 표기 완벽 일치, 개조식)
            ### 2. 목표 대학(또는 범용) 평가 기준에 비추어 볼 때 보완이 필요한 약점 (엄선 압축, 출처 완벽 일치, 개조식)
...
<div style='text-align: center; color: gray; padding: 20px; font-size: 13px;'>
    🏫 학교생활기록부 분석 시스템 v6.5 (나열형 서술 차단 및 핵심 압축 패치)<br>
