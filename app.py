...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            # --- 💡 [프롬프트 대수술] 단순 요약 금지, 심층 분석 톤 복원 및 정답 템플릿 제공 ---
            prompt = f"""
            ...
            1. 이중 출처 표기 및 압축: 한 문단에 모든 과목을 다 때려 넣지 마세요. 테마별로 가장 훌륭한 활동 **2~3개만 엄선**하세요.
            2. 학년/과목을 절대 슬래시(/)나 쉼표(,)로 묶어서 표기하지 마세요. (예: [1, 2학년 진로] ❌ -> [1학년 진로활동, 2학년 진로활동] ⭕)
            3. 특정 대학명 노출 절대 금지: 동국대학교 등 대학 이름이 결과물에 직접 등장하면 안 됩니다.
            4. 개조식 어미 사용: 모든 문장 끝은 '~함', '~임', '~됨', '~판단됨' 으로 명사형 종결할 것.
