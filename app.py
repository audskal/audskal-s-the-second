...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            # --- 💡 [프롬프트 대수술] 출처 100% 일치 강제 규칙 및 예시 추가 ---
            prompt = f"""
            ...
            *(주의: 소제목 옆의 [1학년 기술·가정, 2학년 동아리활동]과 본문의 꼬리표 2개가 1:1로 완벽히 일치합니다. 중간에 다른 과목이 절대 끼어들지 않았습니다.)*
            ...
            ### 1. 목표 대학 및 전공 적합성 주요 경쟁력 (테마별 엄선, 평가적 분석 서술, 100% 일치하는 분리 출처, 개조식)
            ### 2. 목표 대학(또는 범용) 평가 기준에 비추어 볼 때 보완이 필요한 약점 (엄선된 약점 분석, 100% 일치하는 분리 출처, 개조식)
...
<div style='text-align: center; color: gray; padding: 20px; font-size: 13px;'>
    🏫 학교생활기록부 분석 시스템 v6.8 (출처 100% 일치 강제 패치)<br>
