...
with col1:
    ...
    st.info("💡 나이스(NEIS) 원본 PDF는 보안상 안 읽히는 경우가 있습니다. 가급적 아래 빈칸에 내용을 직접 긁어서 붙여넣으세요!")
...
if submit_btn:
    if not api_key:
        ...
        try:
            ...
            ### 2. 범용 평가 기준에 비추어 볼 때 보완이 필요한 부분 (특정 활동 선 언급 후 구체적 보완책 제시, 100% 일치하는 분리 출처, 개조식 종결) ※ 부재중인 학년의 기록 부족 지적 불가!
            ### 3. 추천 탐구 주제 및 면접 예상 질문 3가지 (※ 면접 질문은 반드시 '~다/~까/~요/~오' 형태의 완결된 질문 문장으로 종결!)
            ### 4. 맞춤형 추천 도서 및 연계 활동 제안 (교과명 분류, 도서명(저자) 형식, **연계 활동:** 명시적 서술 필수, 개조식 종결!)
            ### 5. 종합 의견 (※ 반드시 개조식 종결! '~합니다/~됩니다' 경어체 절대 금지!)
            """
