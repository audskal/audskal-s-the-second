    ...
    if not api_key:
        ...
        try:
            ...
            2. 🚫 [문장별 개별 꼬리표 절대 누락 금지!]:
               ...
               - 본문을 작성할 때, 학생의 활동을 언급하는 **모든 문장의 끝에는 반드시 해당 활동의 개별 출처 꼬리표(예: [1학년 진로활동])를 달아야 합니다.** 문장 끝에 꼬리표를 빼먹고 줄글만 쓰는 것은 최악의 오류입니다.
               - 소제목 옆에 선언한 전체 출처 목록과, 본문 문장 끝에 달린 개별 꼬리표들의 집합은 100% 일치해야 합니다. 즉흥적 추가나 누락을 금지합니다.

            3. 🚫 [도서 추천 출력 양식 (실제 책 제목 사용 및 구체적 소개!)]:
               - 참고 자료의 카테고리명(예: 물리 신소재)을 책 제목으로 착각하지 마세요. 반드시 세부 본문 안에서 실제 책 제목(예: <쓸모의 과학, 신소재>)을 찾으세요.
               - 지정된 포맷: "과목명: 순차번호. 도서명(저자명 저) - 구체적인 도서 소개 및 추천 이유" (순차 번호는 1번부터 빠짐없이 정렬)

            4. 개조식 어미 사용: 문장 끝은 '~함', '~임', '~됨', '~판단됨' 으로 명사형 종결할 것.

            💡 [형식 참고용 정답 템플릿 (꼬리표 누락 없는 완벽한 예시)]
            ...
            ### 2. 목표 대학(또는 범용) 평가 기준에 비추어 볼 때 보완이 필요한 약점 (엄선된 약점 분석, 100% 일치하는 분리 출처, 개조식) ※ 부재중인 학년의 기록 부족 지적 불가!
